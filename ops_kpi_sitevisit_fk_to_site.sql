-- =============================================================================
-- ops_kpi_sic: drop FK to ops_kpi_availability, normalize site_id to
-- public.site.site_id, merge duplicate (site_id, date) with SUM(sic_count),
...
--    pg_dump "$DATABASE_URL" --no-owner --table=ops_kpi_sic -Fc -f ops_kpi_sic_pre_fk.dump
--    # or plain SQL:
--    pg_dump "$DATABASE_URL" --no-owner --table=ops_kpi_sic -f ops_kpi_sic_pre_fk.sql
--
-- 2) In-database copy (quick rollback):
--    DROP TABLE IF EXISTS ops_kpi_sic_backup_pre_fk;
--    CREATE TABLE ops_kpi_sic_backup_pre_fk AS TABLE ops_kpi_sic WITH DATA;
--
--    Restore from copy if needed:
--    TRUNCATE ops_kpi_sic;
--    INSERT INTO ops_kpi_sic SELECT * FROM ops_kpi_sic_backup_pre_fk;
...
--    SELECT COUNT(*) AS n_rows, COALESCE(SUM(sic_count), 0)::bigint AS sum_sic
--    FROM ops_kpi_sic;
--
...
-- SELECT COUNT(*) AS n_rows, COALESCE(SUM(sic_count), 0)::bigint AS sum_sic FROM ops_kpi_sic;
...
-- Optional diagnostic: SIC rows whose key does not match any site.site_id and
...
-- Collapse duplicate (site_id, date) preserving total sic_count (idempotent).
CREATE TEMP TABLE _ops_kpi_sic_merged ON COMMIT DROP AS
...
BEGIN
    ...
    IF n > 0 THEN
        RAISE EXCEPTION 'ops_kpi_sic has % rows with no matching site.site_id; fix data before FK', n;
    END IF;
END $$;

ALTER TABLE ops_kpi_sic
    DROP CONSTRAINT IF EXISTS fk_ops_kpi_sic_site;

ALTER TABLE ops_kpi_sic
    ADD CONSTRAINT fk_ops_kpi_sic_site
